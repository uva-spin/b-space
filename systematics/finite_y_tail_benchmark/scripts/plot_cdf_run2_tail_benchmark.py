#!/usr/bin/env python3
"""Plot DYTurbo/MCFM agreement for the available CDF Run II tail benchmarks."""
...
def main() -> None:
    summary = SYS / "summaries" / "cdf_run2_dyturbo_mcfm_canonical.csv"
    if not summary.exists():
        raise FileNotFoundError(f"Run summarize_existing_tail_benchmarks.py first: {summary}")
    df = pd.read_csv(summary).sort_values("qT_over_Q")

    ratio = df["dyturbo_pb_per_GeV"] / df["mcfm_pb_per_GeV"]
    ...
    ax.axhspan(0.95, 1.05, color="#2f7d4b", alpha=0.12, lw=0, label=r"$\pm 5\%$ agreement gate")
    ...
    for _, row in df.iterrows():
        ax.text(
            row["qT_over_Q"],
            row["dyturbo_pb_per_GeV"] / row["mcfm_pb_per_GeV"] + 0.004,
            str(row["row_id"]).split(":")[-1],
            ha="center",
            va="bottom",
            fontsize=9,
            color="0.25",
