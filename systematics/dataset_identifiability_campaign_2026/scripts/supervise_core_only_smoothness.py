#!/usr/bin/env python3
"""Run and audit the isolated core-only FNP smoothness multistart test."""
...
def tag(source_seed: int) -> str:
    return f"coreonly_logcurv5em5_fslope4em3_xslope3em4_init{source_seed}"


def command(source_seed: int) -> list[str]:
    initial = (
        BASE / "outputs"
...
def main() -> None:
    log_dir = BASE / "logs/core_only_smoothness"
    target = BASE / "summaries/core_only_smoothness_multistart"
    ...
    for source_seed in SOURCE_SEEDS:
        ...
        if not (run / "fit_status.json").exists():
            with (log_dir / f"{tag(source_seed)}.log").open("w") as stream:
                subprocess.run(
                    command(source_seed), stdout=stream,
                    stderr=subprocess.STDOUT, text=True, check=True)
        status = json.loads((run / "fit_status.json").read_text())
        final = status["final"]
        rows.append({
            "source_seed": source_seed,
            "run_tag": tag(source_seed),
    ...
    summary = {
        "status": "isolated_core_only_smoothness_multistart_complete",
        ...
        "primary_measure": "cross-start F_NP distribution",
        "remote_transform_closure_in_objective": False,
        "production_sources_modified": False,
    }
    (target / "fnp_curves.csv").write_text(wide.to_csv())
