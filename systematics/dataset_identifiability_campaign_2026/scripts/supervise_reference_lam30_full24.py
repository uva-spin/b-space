#!/usr/bin/env python3
"""Build the full 24-start stationary cross-fit lambda-30 ensemble."""
...
TARGET = BASE / "summaries/crossfit_reference_lam30_full24_stationary"
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        for chunk in range(1, MAX_CHUNKS + 1):
            ...
            if passed:
                break
            previous = target
        endpoint_tags.append(endpoint.name)
    ...
    all_pass = all(any(r["seed"] == s and r["fnp_plateau_gate_pass"] for r in records) for s in SEEDS)
    summary = {
        "status": "complete",
