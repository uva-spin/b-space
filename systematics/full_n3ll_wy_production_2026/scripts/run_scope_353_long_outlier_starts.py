#!/usr/bin/env python3
"""Long-horizon continuation checks for early-stopping start outliers."""
...
def main() -> None:
    ...
    ap.add_argument("--seeds", nargs="+", type=int, default=[310, 311, 319, 322])
    ...
    for seed in args.seeds:
        out = REPORTS / f"scope_353_coupled_fnp_fit_lambda1_candidate_s{seed}_csvnorm_long"
        log = REPORTS / f"scope_353_coupled_fnp_fit_lambda1_candidate_s{seed}_csvnorm_long.log"
    ...
    statuses.sort(key=lambda x: x["seed"])
    result = {
        "status": "isolated_scope_353_long_outlier_start_batch_complete" if all(x["returncode"] == 0 for x in statuses) else "isolated_scope_353_long_outlier_start_batch_incomplete",
        "seeds": args.seeds, "epochs": args.epochs, "patience": args.patience,
        "runs": statuses, "frozen_production_modified": False, "promotion_authorized": False,
    }
    (REPORTS / "scope_353_long_outlier_start_batch_status.json").write_text(json.dumps(result, indent=2) + "\n")
