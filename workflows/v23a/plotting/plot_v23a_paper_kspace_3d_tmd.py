...
def main() -> None:
    ...
    for i, flavor in enumerate(args.flavors, start=1):
        ...
        ax.set_zlabel(z_label(args.quantity, flavor), labelpad=11, fontsize=13)
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=axes, fraction=0.027, pad=0.04, shrink=0.72)
    cbar.set_label(r"relative 68\% half-width", fontsize=12)
    ...
    fig.text(
        ...
        (
            ...
            "interpolated in $\\log x$ between exact support points. "
    ...
    fig.subplots_adjust(left=0.02, right=0.91, bottom=0.09, top=0.93, wspace=0.03)
