...
mkdir -p "${OUT_ROOT}/outputs" "${OUT_ROOT}/logs" "${OUT_ROOT}/norm_inits"
...
run_seed() {
  ...
  [[ -f "${NORM_INIT}" ]] || { echo "Missing norm init ${NORM_INIT}" >&2; exit 1; }
...
export -f run_seed
export OUT_ROOT TRAIN ROOT W_GRID Y_GRID DEVICE CENTRAL_RUN

printf '%s\n' "${SEEDS[@]}" \
  | xargs -n 1 -P "${MAX_PARALLEL}" bash -c 'run_seed "$1"' _

echo
echo "Cached CUDA lambda=3 replica run complete."
echo
echo "Audit with:"
