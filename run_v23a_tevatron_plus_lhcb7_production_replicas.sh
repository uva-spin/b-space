...
# + Tevatron low-qT set plus DYTurbo-fiducial LHCb_7.

ROOT="$(pwd)"
export PYTHONPATH="${ROOT}:${PYTHONPATH:-}"

PYTHON="${PYTHON:-/home/dustin/miniforge3/envs/pdf-fit/bin/python}"
CACHE_ENV="${CACHE_ENV:-${ROOT}/outputs/v23a_tevatron_plus_lhcb7_fidacc_merged_cache/backend_cache/cache_paths.env}"
TRAIN="${TRAIN:-${ROOT}/v21_tail_release_amp0p019_candidate/train_bt_dnn_v21_replica_stable.py}"
PREP="${PREP:-${ROOT}/v23/tools/prepare_v23_replica_norm_inits_fixed_norm_safe.py}"
BACKEND="${BACKEND:-${ROOT}/v23/backends/bt_internal_css_backend_v22_tevatron.py}"
DATA_DIR="${DATA_DIR:-${ROOT}/Data/v23a_tevatron_plus_lhcb7_fiducial_candidate}"
DATASETS="${DATASETS:-E288_200 E288_300 E288_400 E605 E772 CDF_RUN_1 CDF_RUN_2 D0_RUN_1 LHCb_7}"
...
{
  "role": "production experimental replicas for validated v23a fixed-target plus Tevatron low-qT plus DYTurbo-fiducial LHCb_7",
  "central_run": "${CENTRAL_RUN}",
  "data_dir": "${DATA_DIR}",
  "datasets": "$(printf '%s' "${DATASETS}")",
  "excluded": {
    "D0_RUN_2": "normalized spectrum; first-bin quality issue under available treatment",
    "D0_RUN_2N": "fiducial dimuon normalized spectrum; first-bin quality issue under candidate correction",
    "ATLAS_7": "fails controlled central fit under current diagonal-error treatment",
    "qT/Q > production cut": "outside validated low-qT TMD extraction domain"
  },
  "w_grid": "${W_GRID}",
  "y_grid": "${Y_GRID}",
  "backend": "${BACKEND}",
  ...
  "norm_source": "csv",
  "fixed_norm_safe_replica_prep": true,
  "ptp_source": "csv",
  "qT_max_over_Q": ${QTMQ},
...
run_seed() {
  ...
  echo "Running Tevatron + LHCb_7 replica seed ${S}"
...
echo
echo "Tevatron + LHCb_7 production replicas complete."
