#!/usr/bin/env python3
"""Run MCFM LO Z+jet benchmarks for fit-ready Tevatron rows."""
...
def parse_unit(log: Path) -> str:
    text = log.read_text(errors="replace")
    matches = re.findall(r"Value of integral is\s+[-+0-9.Ee]+(?:\s*±\s*[-+0-9.Ee]+)?\s+([fp]b)", text)
    return matches[-1] if matches else "pb"
...
def main() -> None:
    ...
    ap.add_argument("--out", default="outputs/tevatron_mcfm_benchmark_cdf_run2")
    ...
    ap.add_argument("--calls", type=int, default=1000000)
    ...
    rundir = "tevatron_mcfm_benchmark"
        ...
        with log.open("w") as fh:
            subprocess.run(
                [
                    "/bin/bash",
                    "-lc",
                    f"ulimit -s unlimited; exec {shlex.quote(str(mcfm_exe))} {shlex.quote(work_card.name)}",
                ],
                cwd=mcfm_bin,
                env=env,
                stdout=fh,
