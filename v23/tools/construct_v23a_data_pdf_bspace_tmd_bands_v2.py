#!/usr/bin/env python3
"""Construct b-space TMD bands from a plan containing run_dir and pdf_member.

This v2 tool supports two modes:

  1. true data x PDF refit plans:
       each run was trained with the same pdf_member in the plan;
  2. fast PDF-overlay plans:
       each run is an existing fixed-PDF data-replica fit, and the plan-assigned
       pdf_member is used only when reconstructing the TMD grid.

By default this script TRUSTS THE PLAN'S pdf_member.  That is important for
overlay plans because old fixed-PDF runs may have metrics.json saying
pdf_member=0 even though the intended overlay member is nonzero.

The output bands are q16/q50/q84 over the reconstructed TMD grids.
"""
...
def meta_pdf_member(run: Path) -> int | None:
...
def run_grid_tool(args: argparse.Namespace, run: Path, seed: str, member: int, grid_dir: Path) -> None:
    ...
    print(f"grid seed={seed} pdf={member}: {grid_dir}", flush=True)
...
def main() -> None:
    ...
    ap.add_argument("--pdf-member-source", choices=["plan", "meta"], default="plan",
                    help="Use plan pdf_member by default. Use meta only for strict true-refit checking.")
    ap.add_argument("--strict-meta-pdf-match", action="store_true",
                    help="Fail if run metadata contains a pdf_member that differs from the plan.")
    args = ap.parse_args()
    ...
    for _, r in plan.iterrows():
        ...
        seed = get_seed(run, int(r["seed"]) if not pd.isna(r["seed"]) else None)
        plan_member = int(r["pdf_member"])
        meta_member = meta_pdf_member(run)

        if args.strict_meta_pdf_match and meta_member is not None and int(meta_member) != plan_member:
            raise SystemExit(f"PDF mismatch for {run}: plan={plan_member}, metadata={meta_member}")

        member = int(meta_member) if args.pdf_member_source == "meta" and meta_member is not None else plan_member
        ...
        g["plan_pdf_member"] = int(plan_member)
        g["metadata_pdf_member"] = np.nan if meta_member is None else int(meta_member)
        g["run_dir"] = str(run)
        rows.append(g)
        ...
        replica_meta.append({
            ...
            "plan_pdf_member": int(plan_member),
            "metadata_pdf_member": None if meta_member is None else int(meta_member),
            "run_dir": str(run),
            "grid_dir": str(grid_dir),
        })

    long = pd.concat(rows, ignore_index=True)
    keep = ["seed", "pdf_member", "plan_pdf_member", "metadata_pdf_member", "run_dir", "pid", "flavor", "x", "Q", "bT"]
    ...
    manifest = {
        ...
        "pdf_member_source": args.pdf_member_source,
        "strict_meta_pdf_match": bool(args.strict_meta_pdf_match),
        "pdf_members": sorted({m["pdf_member"] for m in replica_meta}),
        "replicas": replica_meta,
        "definition": (
            "Bands are q16/q50/q84 over TMD grids reconstructed from each run's fitted F_NP "
            "and the selected PDF member. With --pdf-member-source plan, this supports fast "
            "experimental-data-replica x PDF-member overlay without cross-section retraining."
        ),
        "limitation": (
            "If runs were not actually trained with these PDF members, this is a PDF-overlay TMD "
            "uncertainty, not PDF-through-refit uncertainty."
        ),
        "outputs": {
            "long": str(long_path),
            "bands": str(band_path),
    ...
    print("\n=== v23a data/PDF b-space TMD bands created ===")
    print(json.dumps({k: manifest[k] for k in ["n_replicas", "pdf_set", "pdf_member_source", "pdf_members", "definition", "limitation"]}, indent=2))
