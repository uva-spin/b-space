...
def reproduce_replica(
    ...
    for dataset, subset in df.groupby("dataset", sort=False):
        ...
        z = float(rng.normal())
        factor = 1.0 + z * norm_rel if norm_rel > 0.0 else 1.0
        target[indices] = factor * central[indices]
        draws[str(dataset)] = {
            "z": z,
            "norm_rel": norm_rel,
            "replica_norm_factor": factor,
            "fixed_normalization": bool(norm_rel <= 0.0),
        }
...
def profiled_scales(
    ...
    for dataset, subset in df.groupby("dataset", sort=False):
        ...
        if norm_rel <= 0.0:
            ...
            inv_delta2 = 1.0 / norm_rel**2
            denominator = float(np.sum(weights * np.square(r)) + inv_delta2)
            numerator = float(np.sum(weights * r * t) + inv_delta2)
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        diagnostics = {
            ...
            "note": "Datasets with norm_rel <= 0 are treated as fixed-normalization: replica norm factor 1, profiled norm scale 1, pull 0.",
        }
        ...
        summary_rows.append({
            ...
            "n_fixed_normalization": int(norms["fixed_normalization"].sum()),
        })
