#!/usr/bin/env python3
"""Run DYTurbo fixed-order V+jet benchmarks for fit-ready Tevatron rows."""
...
def card_text(row: pd.Series, *, output_name: str, pdf_set: str, pdf_member: int, cores: int,
    ...
    return f"""# Tevatron DYTurbo fixed-order V+jet benchmark
# Row: {row.row_id}
# Dataset: {row.dataset}
    ...
    for line in text.splitlines():
        ...
        if len(parts) >= 2:
            try:
                ...
                continue
    raise ValueError(f"Could not parse DYTurbo text output: {path}")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--data", default="Data/v23a_fixed_target_plus_tevatron_absolute_fit_ready/CDF_RUN_2.csv")
    ap.add_argument("--rows", nargs="+", default=["CDF_RUN_2:17", "CDF_RUN_2:18", "CDF_RUN_2:22", "CDF_RUN_2:28"])
    ap.add_argument("--out", default="outputs/tevatron_dyturbo_benchmark_cdf_run2")
    ...
    env = os.environ.copy()
    lib_parts = [
        str(DEFAULT_CONDA_LIB),
        str(Path(args.dyturbo_root) / "lib"),
    ...
    for _, row in selected.iterrows():
        ...
        output_name = f"{tag}_dyturbo_vj_nlo"
        card = (cards / f"{output_name}.in").resolve()
        card.write_text(
            card_text(row, output_name=output_name, pdf_set=args.pdf_set, pdf_member=args.pdf_member,
                      cores=args.cores, mu_r_factor=args.mu_r_factor,
                      mu_f_factor=args.mu_f_factor, seed=args.seed)
        )
