...
class TrainConfig:
    ...
    dataset_norm_lr: float = 0.0
    replica_train_scope: str = "all"
    lambda_replica_logf_anchor: float = 0.0
    replica_anchor_x_values: tuple[float, ...] = (
        0.15, 0.20, 0.30, 0.40, 0.50,
    )
    replica_anchor_bmin: float = 0.0
    replica_anchor_bmax: float = 8.0
...
def fit_model(model: PrecomputedKernelModel, data: TensorData, cfg: TrainConfig) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    scope = str(cfg.replica_train_scope).lower()
    if scope not in ("all", "head", "none"):
        raise ValueError(
            "replica_train_scope must be one of: all, head, none"
        )

    if scope in ("head", "none"):
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        if scope == "head":
            for parameter in model.np_factor.head.parameters():
    ...
    if norm_params:
        norm_lr = (
            float(cfg.dataset_norm_lr)
            if float(cfg.dataset_norm_lr) > 0.0
            else float(cfg.lr)
        )
        parameter_groups.append({
            "params": norm_params,
    ...
    if not parameter_groups:
        raise ValueError(
            "No trainable parameters: enable dataset norms or choose "
            "replica_train_scope all/head."
        )

    anchor_x = torch.tensor(
        tuple(float(value) for value in cfg.replica_anchor_x_values),
        dtype=data.Q.dtype,
        device=data.Q.device,
    )
    anchor_mask = (
        (model.b >= float(cfg.replica_anchor_bmin))
        & (model.b <= float(cfg.replica_anchor_bmax))
    )
    if not bool(torch.any(anchor_mask)):
        raise ValueError("Replica anchor bT interval has no grid points")
    anchor_b = model.b[anchor_mask]
    with torch.no_grad():
        anchor_logf_reference = torch.log(
            torch.clamp(
                model.np_factor(anchor_x, anchor_b),
                min=1.0e-30,
            )
        ).detach()

    def replica_anchor_penalty() -> torch.Tensor:
        if float(cfg.lambda_replica_logf_anchor) <= 0.0:
            return torch.zeros(
                (),
                dtype=data.Q.dtype,
                device=data.Q.device,
        ...
        return (
            float(cfg.lambda_replica_logf_anchor)
            * torch.mean((logf - anchor_logf_reference).square())
        )
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opt,
        factor=0.5,
        patience=max(10, cfg.patience // 4) if cfg.patience else 50,
    )
    ...
    # Epoch zero is a real candidate.  This prevents a pseudo-data replica
    # from returning a checkpoint worse than the correctly initialized
    # central model plus profiled normalization nuisances.
    model.eval()
    with torch.no_grad():
        pred_raw_initial = model(
            data.row_index,
            data.Q,
            data.x1,
            data.x2,
        ...
        initial_norm_penalty = float(
            (
                float(cfg.lambda_dataset_norm)
                * dataset_norms.penalty(normalize_by=len(data))
            ).detach().cpu()
        )
        initial_anchor_penalty = float(
            replica_anchor_penalty().detach().cpu()
        )
        initial_rel_rmse = float(
            torch.sqrt(
                torch.mean(
                    (
                        (pred_initial - data.target)
                        / torch.clamp(
                            torch.abs(data.target),
                            min=1.0e-12,
                        )
                    ).square()
                )
        ...
        initial_rmse = float(
            torch.sqrt(
                torch.mean((pred_initial - data.target).square())
            ).detach().cpu()
        )
        initial_global_norm = float(
            model.global_norm().detach().cpu()
        )

    best = initial_chi2
    best_objective = (
        initial_chi2
        + initial_norm_penalty
        + initial_anchor_penalty
    )
    best_epoch = 0
    best_state = {
    ...
    best_norm_state = {
        key: value.detach().cpu().clone()
        for key, value in dataset_norms.state_dict().items()
    }
    history.append({
        "epoch": 0.0,
        "batch_loss": best_objective,
        "objective": best_objective,
        "reg_multiplier": float(_reg_multiplier_for_epoch(0, cfg)),
        "chi2_like": initial_chi2,
        "relative_rmse": initial_rel_rmse,
        "rmse": initial_rmse,
        "lr": float(cfg.lr),
        "global_norm": initial_global_norm,
        "norm_penalty": initial_norm_penalty,
        "replica_anchor_penalty": initial_anchor_penalty,
    })
    print(
        "epoch     0/{}: chi2_like={:.6g}, objective={:.6g}, "
        "norm_penalty={:.3g}, anchor={:.3g}, scope={}".format(
            cfg.epochs,
            initial_chi2,
            best_objective,
            initial_norm_penalty,
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for start in range(0, n, batch_size):
            ...
            loss_anchor = replica_anchor_penalty()
            loss = loss_data + loss_reg + loss_norm + loss_anchor
            loss.backward()
            if cfg.grad_clip and cfg.grad_clip > 0 and model_params:
        ...
        with torch.no_grad():
            ...
            objective = chi2_like + norm_penalty + anchor_penalty
        ...
        history.append({
            ...
            "replica_anchor_penalty": anchor_penalty,
        })
        if epoch == 1 or epoch % log_every == 0 or epoch == int(cfg.epochs):
            ...
            print(
                ...
                f"chi2_like={chi2_like:.6g}, objective={objective:.6g}, "
                f"anchor={anchor_penalty:.3g}, rel_rmse={rel_rmse:.6g}, "
                f"lr={lr_now:.3g}, norm={gnorm:.4g}, "
                f"norm_penalty={norm_penalty:.3g}{ds_txt}",
                flush=True,
            )
        improved = objective < best_objective - float(cfg.min_delta)
        if improved:
            best = chi2_like
    ...
    if bool(cfg.restore_best) and best_state is not None:
        ...
        restored_best = True
        print(
            f"restored best epoch {best_epoch} with "
            f"chi2_like={best:.6g}, objective={best_objective:.6g}",
            flush=True,
        )
    meta = {
        "seconds": time.time() - t0,
        "n_points": n,
        "epochs_run": max(0, len(history) - 1),
        ...
        "final_relative_rmse": float(history[best_epoch]["relative_rmse"] if restored_best else history[-1]["relative_rmse"]),
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="Separate learning rate for dataset-normalization nuisances; <=0 uses --lr.",
    )
    ap.add_argument(
        "--replica-train-scope",
        choices=["all", "head", "none"],
        default="all",
        help="For replica stability: train all NP parameters, only the final NP head, or no NP parameters.",
    )
    ap.add_argument(
        "--lambda-replica-logf-anchor",
        type=float,
        default=0.0,
        help="Function-space trust-region penalty anchoring replica log F_NP to its initial central checkpoint.",
    )
    ap.add_argument(
        "--replica-anchor-x-values",
        type=float,
        nargs="+",
        default=[0.15, 0.20, 0.30, 0.40, 0.50],
    )
    ap.add_argument("--replica-anchor-bmin", type=float, default=0.0)
    ap.add_argument("--replica-anchor-bmax", type=float, default=8.0)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    train_cfg = TrainConfig(
        ...
        dataset_norm_lr=float(args.dataset_norm_lr),
        replica_train_scope=str(args.replica_train_scope),
        lambda_replica_logf_anchor=float(args.lambda_replica_logf_anchor),
        replica_anchor_x_values=tuple(
            float(value) for value in args.replica_anchor_x_values
        ),
        replica_anchor_bmin=float(args.replica_anchor_bmin),
        replica_anchor_bmax=float(args.replica_anchor_bmax),
    )
